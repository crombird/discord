...
import * as messages from "./roles-delete-prompt.intl";

export default defineCommand({
  definition: {
    type: ApplicationCommandType.Message,
    name: "Roles: Delete prompt",
  ...
  select(interaction): interaction is APIMessageApplicationCommandInteraction {
    return (
      ...
      interaction.data.name === "Roles: Delete prompt"
    );
  },
  ...
  async handle(interaction, context) {
    ...
    if (
      ...
      return {
        ...
        data: {
          ...
          content:
            "*This command is meant to be used on prompt messages created by **/roles prompt**.*",
        },
      };
    }
    ...
    if (!(BigInt(interaction.app_permissions) & PermissionFlagsBits.ViewChannel)) {
      return {
        ...
        data: {
          flags: MessageFlags.Ephemeral,
          content:
            "*Crom doesn't have permissions to delete its own messages. Run **/roles manage**.*",
        },
      };
    }
    ...
    return {
      ...
      data: {
        flags: MessageFlags.Ephemeral,
        content: "_Prompt message deleted._",
      },
    };
  },
});
